<template>
  <div class="container">
    ...
    <el-button @click="rowData">新增</el-button>
    <hot-table v-if="settings.data.length" :settings="settings"  ref="hotTableComponent" :height=" heights-240"></hot-table>
  </div> 
</template>
<script>
...
import {report} from '@/api/cube.js'
  ...
  export default {
    ...
    methods:{
        ...
        swapHotData() {
            // 获取一列的数据
            let hotTableComponent = this.$refs.hotTableComponent
            if (hotTableComponent) {
                let a = this.$refs.hotTableComponent.hotInstance.getDataAtCol(8)
            console.log(a)
            }
        ...
        dist(){
            ...
            report(item).then((res)=>{
                ...
                //  console.log("a",a)
                this.settings.minRows = this.settings.data
