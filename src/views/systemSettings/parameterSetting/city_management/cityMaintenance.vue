<template>
    <div id="cityMaintenance">
        ...
        border>
            ...
            <el-table-column prop="sname" label="企业名称" width="360" align="left" show-overflow-tooltip="true"></el-table-column>
        ...
        <el-dialog
            ...
            <el-form ref="form" :rules="rules" :model="form" label-width="100px">
                <el-form-item label="企业编码：" prop="scode">
                    <el-input class="dialog_textarea" :rows="2" placeholder="请输入企业编码" v-model="form.scode" :readonly="readonly"></el-input>
                </el-form-item>
                <el-form-item label="企业名称：" prop="sname">
                    <el-input class="dialog_textarea" :rows="2" placeholder="请输入企业名称" v-model="form.sname"></el-input>
                </el-form-item>
            </el-form>
...
import {
    // 查询表格数据接口
    dim_cityenterprise_query_all,
    // 添加企业接口
    dim_cityenterprise_add,
    // 删除企业接口
    dim_cityenterprise_delete,
    // 修改企业接口
    dim_cityenterprise_update
} from '~api/cube.js';
export default {
    props: {
        height: Number
    ...
    methods: {
        ...
        deleteClick(scope){
            ...
            me.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                ...
                dim_cityenterprise_delete(params).then(res => {
                    if(res.data.code === 200){
        ...
        determineClick(form){
            ...
            me.$refs[form].validate((valid) => { 
                if (valid && me.stype==1) {     // 添加状态
                    ...
                    dim_cityenterprise_add(params).then(res => {
                        if(res.data.code === 200){
                            me.dimIndex_query_by_stypeA() ;
                            me.cancelClick("form") ;
                    ...
                    if(me.getIsChanger()) {
                        ...
                        dim_cityenterprise_update(params).then(res => {
                            if(res.data.code === 200){
                                me.dimIndex_query_by_stypeA() ;
                                me.cancelClick("form") ;
