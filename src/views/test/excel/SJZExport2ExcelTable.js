...
function sheet_from_array_of_arrays(data, headNum, columns) { //debugger
    // var ws = {};
    //这个处理只是针对于行次不用处理两位小数。
    ...
    ws['!cols'] = [];
    ws['!rows'] = [];
    for (var n = 0; n != data[0].length; ++n) { // debugger
        if (rowIndexs && rowIndexs.length > 0 && rowIndexs.indexOf(n) != -1) {
            ...
            if(n == 0 || n == 4){
                ...
                ws['!cols'].push({ wpx: 360 });
            }else if(n == data[0].length - 2){
                ws['!cols'].push({ wpx: 220 });
            }else{
                ws['!cols'].push({ wpx: 170 });
            }
        }
    }
    var range = {
...
export function export_table_to_excel(id, name, columns) { //debugger
    ...
    data[0] = data[0].filter(res => { //debugger
        if(res=="操作"){}else{ return res ;}
    }) ;
    for(let i=1; i<data.length; i++){
        data[i] = data[i].filter(res => {
            if(res=="启用修改修改密码公司授权" || res=="禁用修改修改密码公司授权"){}else{ return res ; }
        });
    }
    var ws_name = "SheetJS";
    var wb = new Workbook(),
