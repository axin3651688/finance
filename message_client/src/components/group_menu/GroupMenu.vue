<template>
  <div class="GroupMenu">
    ...
    <relative-pop :showClose="false">
      ...
      <ul slot="pop">
        <li class="pop-list" v-if="loginUserId === groupOwnerId" @click="showGroupSettingDialog=true">
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
    <el-button size="small" @click="quitGroup()">确 认</el-button>
...
export default {
  ...
  computed: {
    ...
    groupOwnerId() {
      return this.groupInfo ? this.groupInfo.ownerId : null;
    }
  },
  methods: {
    ...
    /**
     * 退出群组，先判断是不是管理员，管理员不能直接退出群组，退出群即解散该群
     * TODO：退出或解散群后要在本地的sessionList中移除
     */
    quitGroup() {
      debugger;
      // 如果是管理员则解散群
      if (this.loginUserId === this.groupOwnerId) return this._dissolveGroupConfirm();
      ...
      QUIT_GROUP(params).then(res => {
        ...
        if (res.data.code === 200) {
          this.showGroupQuitDialog = false;
          this.$message({
            type: 'success',
            message: '退出群组:' + res.data.msg,
            showClose: true
          });
        } else {
     ...
     * @private
     */
    _dissolveGroupConfirm() {
      this.$confirm('你是群组，退出后会解散该群, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
