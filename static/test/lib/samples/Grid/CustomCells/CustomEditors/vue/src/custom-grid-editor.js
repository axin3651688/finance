...
export class CustomGridEditor {
    ...
    constructor(flex, binding, edtClass, options) {
        ...
        this._grid.addEventListener(this._grid.hostElement, 'keydown', (e) => {
            ...
            if (e.keyCode == wjcCore.Key.F4 ||
                ...
                let colIndex = this._grid.selection.col;
    ...
    _beginningEdit(grid, args) {
        ...
        let rcCell = grid.getCellBoundingRect(args.row, args.col), rcBody = document.body.getBoundingClientRect(), ptOffset = new wjcCore.Point(-rcBody.left, -rcBody.top), zIndex = (args.row < grid.frozenRows || args.col < grid.frozenColumns) ? '3' : '';
        ...
        // start editing item
        let ecv = grid.editableCollectionView, item = grid.rows[args.row].dataItem;
        ...
        setTimeout(() => {
            ...
            let key = (evt && evt.charCode > 32)
                ? String.fromCharCode(evt.charCode)
                : null;
            // get input element in the control
            let input = this._ctl.hostElement.querySelector('input');
            // send key to editor
            if (input) {
                if (key) {
                    input.value = key;
    ...
    _closeEditor(saveEdits) {
        if (this._rng) {
            let flexGrid = this._grid, ctl = this._ctl, host = ctl.hostElement;
            // raise grid's cellEditEnding event
            let e = new wjcGrid.CellEditEndingEventArgs(flexGrid.cells, this._rng);
    ...
    _commitRowEdits() {
        let flexGrid = this._grid, ecv = flexGrid.editableCollectionView;
        this._closeEditor(true);
        if (ecv && ecv.currentEditItem) {
            let e = new wjcGrid.CellEditEndingEventArgs(flexGrid.cells, flexGrid.selection);
            ecv.commitEdit();
            setTimeout(() => {
